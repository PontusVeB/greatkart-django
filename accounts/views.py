...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Registration completed, we have send you a verification email')
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # get the cart itmes from the user to access his product variations
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Congrats, your acccount is activated.')
        return redirect('login')
